...
func (ms *MessageSystem) Send(pid int, msg []byte /*Message*/) {
	delay := 2 * time.Second
	var conn net.Conn
	var err error
	for i := 1; i <= 5; i++ {
...
func (ms *MessageSystem) Receive() (msg []byte /*Message*/) {
	msg = <-ms.mbox
	return msg
}
	...
	go func() {
		...
		for {
			...
			default:
				...
				checkError(err)
				msg := make([]byte, 1024) // 1KB buffer, adjust size as needed
				_, err = conn.Read(msg)
				if err != nil {
